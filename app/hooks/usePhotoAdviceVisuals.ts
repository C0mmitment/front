...
> = {
  ...
  forward:    { dx:   0, dy:   0, isDepth: true, scaleTarget: 1.1, rotation: "0deg", position: { left: "50%", top: "50%" }, },
  backward:   { dx:   0, dy:   0, isDepth: true, scaleTarget: 0.9, rotation: "0deg", position: { left: "50%", top: "50%" }, },
};
  ...
  useEffect(() => {
    if (config.isDepth) {
      ...
      arrowX.value = withRepeat(
        withSequence(
          withTiming(config.dx, { duration: 400 }),
      ...
      arrowY.value = withRepeat(
        withSequence(
          withTiming(config.dy, { duration: 400 }),
          withTiming(0, { duration: 400 })
        ),
        -1,
        true
